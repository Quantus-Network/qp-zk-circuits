...
impl Layer0AggregationArtifacts {
    pub fn new_from_binaries_dir<P: AsRef<Path>>(bins_dir: P) -> Result<Self> {
        ...
        let inner_artifacts = InnerAggregationArtifacts::new_from_binaries_dir(&bins_dir)
            .context("failed to load compact-child inner artifacts")?;
...
impl Layer0AggregationProver {
    ...
    pub fn commit(mut self, proofs: Vec<Proof>) -> Result<Self> {
        ...
        if proofs.len() > self.artifacts.num_leaf_proofs() {
            bail!(
                "too many proofs for compact-child 2x8 layer-0: got {}, expected at most {}",
                proofs.len(),
...
mod tests {
    ...
    use crate::layer0::circuit::constants::{
        ASSET_ID_START, BLOCK_HASH_START, BLOCK_NUMBER_START, LEAF_PI_LEN, NULLIFIER_START,
        OUTPUT_AMOUNT_1_START, VOLUME_FEE_BPS_START,
    };
    use plonky2::field::types::Field;
    use test_helpers::fake_leaf::{build_fake_leaf_circuit, prove_fake_leaf};

    #[test]
    fn normalize_proofs_for_inner_split_keeps_real_proof_in_group_a_slot_zero() {
        let (data, targets) = build_fake_leaf_circuit();
        let dummy = prove_fake_leaf(&data, &targets, dummy_leaf_pi());
        let real = prove_fake_leaf(&data, &targets, real_leaf_pi());
        let mut proofs = vec![dummy.clone(), dummy, real.clone()];
    ...
    fn real_leaf_pi() -> [F; LEAF_PI_LEN] {
        ...
        public_inputs[ASSET_ID_START] = F::from_canonical_u64(7);
        public_inputs[OUTPUT_AMOUNT_1_START] = F::from_canonical_u64(11);
        public_inputs[VOLUME_FEE_BPS_START] = F::from_canonical_u64(42);
