...
use zk_circuits_common::circuit::{
    wormhole_aggregator_circuit_config, wormhole_leaf_circuit_config, C, D, F,
};
...
pub fn build_leaf_context(report: &mut PhaseReport) -> Result<LeafContext> {
    use wormhole_circuit::circuit::circuit_logic::WormholeCircuit;

    report.phase_start("build_leaf_circuit");
    let circuit = WormholeCircuit::new(wormhole_leaf_circuit_config());
...
pub fn generate_leaf_proof(
    ...
    let prover = WormholeProver::new(wormhole_leaf_circuit_config());
