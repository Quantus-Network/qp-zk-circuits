...
\]
Because this address is derived from a hash rather than a public key, no one can spend from it via ordinary signature-based authorization. Funds sent here are effectively burned. Anyone can deposit to a wormhole address through an ordinary on-chain transfer, creating a record that can later be authenticated inside the circuit. A user may receive funds from their own prior transfers or from third-party payments to wormhole addresses derived from secrets the user controls.
...
\bibitem[Plonky3 Contributors(2026)]{plonky3}
...
\newblock Software repository, 2026.
\newblock \url{https://github.com/quantus-network/qp-zk-circuits}
