...
\begin{proof}
By knowledge soundness, if \(\mathsf{Verify}(\pi, x) = 1\), there exists an extractor that outputs a witness \(w\) satisfying conditions (C1)--(C5) with probability \(1 - \epsilon_{\mathrm{ks}}\). Condition (C3) requires the witness to include a valid ZK tree inclusion proof for a deposit to address \(\WA(s)\), and (C4) binds this proof to the block \((h_B, b)\) via the committed tree root. If no such deposit exists, the extractor must either fail (breaking knowledge soundness) or output a forged inclusion path, which requires a collision in the tree's Poseidon hash (probability \(\epsilon_{\mathrm{coll}}\)). The blockchain's validation that \((h_B, b)\) refers to a real block completes the binding.
\end{proof}
