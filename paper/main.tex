...
The user batches leaf proofs that share the same \texttt{block\_hash}, asset, and fee rate, pads with dummy leaves if needed, shuffles the order, and produces a zero-knowledge wrapper proof that forwards grouped exits and canonically sorted nullifiers. The wrapper enforces the fee inequality once over the sums of all real inputs and outputs in the batch. Each leaf retains one input and two output slots, but an individual leaf's outputs need not fit within that leaf's input; value may be pooled across leaves as long as the private batch is solvent in aggregate. A user may therefore combine multiple deposits from distinct wormhole addresses into one private settlement segment without exposing the deposit-to-output amount assignment. This step should be performed locally to preserve privacy.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item The resulting nullifiers are sorted in canonical digest order, independently of the exit-slot layout.
\end{itemize}
...
A first-layer aggregate \(Y\) exposes block number \(b\), asset identifier \(a\), fee rate \(f\), batch capacity \(N\), a canonically sorted list of \(N\) nullifiers, and grouped exits \(\{(\alpha_j, e_j)\}_{j=1}^{E}\) with total \(S = \sum_{j=1}^{E} e_j\). For candidate-set analysis the sorted nullifier list is equivalently treated as a multiset.
\end{definition}
...
The implemented layout does not expose a positional nullifier-to-exit association. After dummy replacement, the private-batch circuit sorts all nullifier digests into canonical order, while exits are grouped in a separate region. A non-zero exit slot therefore cannot mark any particular sorted nullifier as real. The exits alone reveal the unavoidable lower bound \(\lceil E/2\rceil\) on the number of real leaves, because each leaf has two slots; the nullifier ordering adds no slot-specific leakage.
