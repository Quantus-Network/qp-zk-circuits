...
\]
and produces a leaf proof. Its intermediate public interface contains the nullifier, the exits, the output amounts, the asset identifier, the fee basis points, the block reference, and the authenticated input amount. This interface is consumed only by the user's local first-layer wrapper; leaf proofs are not submitted to the chain. The wrapper's zero-knowledge proof hides the input amounts, secrets, and ZK tree inclusion proofs.

\paragraph{4. First-Layer Aggregation:}
The user batches leaf proofs that share the same \texttt{block\_hash}, asset, and fee rate, pads with dummy leaves if needed, shuffles the order, and produces a zero-knowledge wrapper proof that forwards grouped exits and a privately permuted nullifier list. A fixed network of private Boolean switches proves that the public list is exactly the selected real/dummy nullifier multiset without modifying any digest; the switch witness is not exposed. The wrapper enforces the fee inequality once over the sums of all real inputs and outputs in the batch. Each leaf retains one input and two output slots, but an individual leaf's outputs need not fit within that leaf's input; value may be pooled across leaves as long as the private batch is solvent in aggregate. A user may therefore combine multiple deposits from distinct wormhole addresses into one private settlement segment without exposing the deposit-to-output amount assignment. This step should be performed locally to preserve privacy.

\paragraph{5. Settlement:}
The blockchain verifies the aggregate proof, checks that the public \texttt{block\_hash} matches the referenced block number, verifies that the fee parameter matches the configured rate, rejects reused nullifiers, and settles the exits. A public batch preserves the boundaries of its constituent private segments. For each accepted segment with total exit \(S_q\), settlement charges \(\lceil S_q f/(10000-f)\rceil\) quantized units and sums those per-segment ceilings; applying one ceiling to the combined public-batch output would not match the rounding boundaries proved by the private segments.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \texttt{input\_amount}: Deposited amount before fee deduction, appended to the intermediate leaf interface and hidden by first-layer aggregation.
\end{itemize}
...
Let \(x = (a, v_1, v_2, f, n, \alpha_1, \alpha_2, h_B, b, v_{\mathrm{in}})\) denote the intermediate public input tuple, with \(v_{\mathrm{in}}\) appended after the pre-existing fields, and let \(w\) denote the witness described above. The leaf relation \(\mathcal{R}_{\mathrm{leaf}}\) holds when there exists a witness \(w\) such that:
...
The leaf circuit range-checks and authenticates \(v_{\mathrm{in}},v_1,v_2\), but deliberately imposes no per-leaf fee or conservation inequality. The two output slots remain fixed; conservation is enforced over their sums in the locally generated private-batch proof. In formulas that use basis points, \(f\) denotes the integer fee parameter. In the privacy analysis we overload \(f\) to denote the fractional rate \(f/10^4\), so volume thresholds take the form \(S/(1-f)\).
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item compute and sum the volume-fee ceiling separately for each accepted private segment
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item The resulting nullifiers pass through a privately witnessed permutation network that preserves their exact multiset independently of the exit-slot layout.
\end{itemize}

We call one first-layer output a \emph{private settlement segment}. For its set \(R\) of real leaves, define
\[
I_q = \sum_{i\in R} v_{\mathrm{in},i}
\qquad\text{and}\qquad
S_q = \sum_{i\in R}(v_{i,1}+v_{i,2}).
\]
In addition to recursive verification, metadata consistency, exit grouping, and nullifier handling, the private-batch relation enforces
\[
S_q\cdot 10000 \leq I_q\cdot(10000-f).
\]
Dummy inputs and outputs are masked to zero before summation. Neither \(I_q\) nor any \(v_{\mathrm{in},i}\) is forwarded to the aggregate proof. The relation preserves the one-input--two-output leaf shape while allowing cross-leaf value pooling: a leaf may carry outputs exceeding its own input if other real inputs in the same segment cover the difference.
...
Higher layers aggregate already-wrapped proofs. Since they operate only on public outputs, they can be safely delegated, and zero-knowledge is not required in principle at these layers. They verify already fee-valid private segments and preserve segment boundaries; they do not introduce a second fee inequality or merge the segments' integer-rounding boundaries.
...
\begin{enumerate}
    ...
    \item \(\mathcal{B}\) outputs a leaf proof \(\pi\) and its intermediate public input tuple \(x\) as defined in \cref{sec:formal}.
    \item \(\mathcal{B}\) wins if \(\mathsf{Verify}(\pi, x) = 1\) and there is \emph{no} witness \(w = (s, c, \ldots)\) satisfying \(\mathcal{R}_{\mathrm{leaf}}(x, w)\) whose authenticated deposit---the ZK tree leaf recording a transfer to \(\WA(s)\) with counter \(c\), asset \(a\), and input amount \(v_{\mathrm{in}}\)---is actually recorded in the ZK tree at block \((h_B, b)\).
\end{enumerate}
...
\begin{proof}
Suppose \(\mathcal{B}\) wins. By knowledge soundness, the extractor outputs a witness \(w\) satisfying \(\mathcal{R}_{\mathrm{leaf}}(x, w)\)---and hence conditions (C1)--(C4)---except with probability \(\epsilon_{\mathrm{ks}}\). Because \(\mathcal{B}\) wins, no satisfying witness has its deposit recorded in the tree; in particular, the extracted witness's deposit tuple \((\WA(s), c, a, v_{\mathrm{in}})\) is absent from the ZK tree at block \((h_B, b)\). Yet condition (C3) requires \(w\) to include a ZK tree inclusion path for exactly this tuple that verifies against the root committed in \((h_B, b)\) per (C4). A verifying path for an absent leaf requires a collision in the tree's Poseidon hash (probability \(\epsilon_{\mathrm{coll}}\)). The blockchain's validation that \((h_B, b)\) refers to a real block ensures the committed root is the correct one for that height.
\end{proof}
...
\begin{enumerate}
    ...
    \item \(\mathcal{B}\) submits a sequence of valid private segments resulting in withdrawals totaling value \(W\).
    \item \(\mathcal{B}\) wins if \(W\cdot 10000 > V\cdot(10000-f)\), where \(f\) is the common fee rate in basis points.
\end{enumerate}
\end{definition}
...
\begin{itemize}
    \item Each accepted private segment \(q\) enforces \(S_q\cdot10000 \leq I_q\cdot(10000-f)\) over its real leaves.
    \item Aggregation preserves output sums; grouping outputs to identical destinations does not change \(S_q\).
    \item One-time withdrawal (\cref{thm:one-time}) and the blockchain's nullifier check prevent a deposit from contributing to more than one accepted segment.
\end{itemize}
Thus the segment input sets are disjoint and \(\sum_q I_q\leq V\). Summing the segment inequalities gives
\[
W\cdot10000
= \sum_q S_q\cdot10000
\leq \sum_q I_q(10000-f)
\leq V(10000-f),
\]
contradicting the winning condition.

For \(0\leq f<10000\), define the on-chain fee for segment \(q\) as
\[
F_q=\left\lceil\frac{S_q f}{10000-f}\right\rceil.
\]
The proved inequality implies \(S_q+F_q\leq I_q\), so user exits plus the fee distributed or burned by settlement remain covered by that segment's deposits. A public batch must therefore charge \(\sum_q F_q\) over its accepted private segments. Charging \(\left\lceil(\sum_q S_q)f/(10000-f)\right\rceil\) once can be smaller because of integer rounding and does not preserve the exact fee boundary attested by each private segment.
\end{proof}
...
A first-layer aggregate \(Y\) exposes block number \(b\), asset identifier \(a\), fee rate \(f\), batch capacity \(N\), a privately permuted list of \(N\) nullifiers, and grouped exits \(\{(\alpha_j, e_j)\}_{j=1}^{E}\) with total \(S = \sum_{j=1}^{E} e_j\). For candidate-set analysis the nullifier list is treated as a multiset.
\end{definition}
...
\end{enumerate}
The \emph{effective anonymity set} is
\[
\mathcal{A}(Y) = \{ I \subseteq \{1,\ldots,M\} \mid I \text{ is plausible for } Y \}.
\]
These are the amount and cardinality constraints relevant to funding-subset feasibility. There is no longer a cap \(\sum_j x_{ij}\leq a_i(1-f)\) tying the two output slots of leaf \(i\) to that leaf's input: only the segment-wide volume inequality and the \(2|I|\) available slots apply. For example, at zero fee, deposits \(\{100,2\}\) can explain three exits of \(34\) each even though one raw leaf must carry an output larger than its own input; the aggregate totals are both \(102\), and three exits fit in four slots. The true funding subset lies in \(\mathcal{A}(Y)\). The definition depends on exits and metadata only; the aggregate hides the intermediate input amounts, and published nullifiers do not identify deposits without inverting \(\Null(s,c)\).
\end{definition}
...
\Cref{thm:nullifier-indist} addresses a different question: whether an adversary can tell real nullifiers from dummy padding, or infer how many real authorizations appear in a batch. This hides batch composition but does not change the deposit-unlinkability analysis above. Without indistinguishability, an adversary who learns the number of real authorizations could restrict attention to subsets of that exact size, potentially shrinking the effective candidate set below \(k\).

The implemented layout does not expose a positional nullifier-to-exit association when the local prover samples an independent permutation. After dummy replacement, a private Boolean-switch network permutes complete nullifier digests while exits remain in a separate grouped region. The circuit proves exact multiset preservation but does not prove that the user's private choice was random; choosing the identity weakens only that user's segment privacy in the intended local self-batching model. A non-zero exit slot therefore cannot mark any particular permuted nullifier as real. The exits alone reveal the unavoidable lower bound \(\lceil E/2\rceil\) on the number of real leaves, because each leaf has two slots; the nullifier ordering adds no slot-specific leakage.

Because the private-batch relation enforces an inequality rather than exact equality, the adversary must consider any subset whose aggregate deposits can fund at least \(S\) after fees. A user may voluntarily under-withdraw by choosing \(S\) below the segment maximum, enlarging the volume-feasible candidate set; after the chain charges the fee derived from \(S\), the remaining deposit value stays locked at the unspendable addresses and is effectively burned. Beyond aggregate volume, the binding structural constraint is the unchanged two-output slot count, not a public assignment of output amounts to individual deposits.

\paragraph{Complexity of counting plausible subsets:}
The decision version of subset sum is NP-complete \cite{subsetsum}. The counting version \textsc{\#SubsetSum} is \#P-complete \cite{valiant1979}: the standard reduction establishing NP-completeness of subset sum is parsimonious, so it transfers to the counting class. In the special case \(E = 1\) and \(N = M\), condition (ii) is vacuous and \(\mathcal{A}(Y)\) reduces to threshold \textsc{\#SubsetSum} with \(T = S/(1-f)\).
...
\begin{proof}
We reduce from \textsc{\#SubsetSum}. Given an instance \((a_1, \ldots, a_M, T)\), construct an aggregate view with fee rate \(f = 0\) (avoiding any quantization of amounts), \(E = 1\), exit \((\alpha_1, e_1)\) where \(e_1 = T\), batch cap \(N = M\), and deposit pool \(\mathcal{D}(b,a)\) containing the same amounts. For any \(I \subseteq \{1,\ldots,M\}\), condition (iii) holds iff \(\sum_{i \in I} a_i \geq T\), and condition (ii) holds because \(E = 1 \leq 2|I|\) for every non-empty \(I\). Thus \(|\mathcal{A}(Y)|\) equals the number of subsets summing to at least \(T\). Given oracle access to this count, two calls with thresholds \(T\) and \(T+1\) recover \textsc{\#SubsetSum} exactly as in the standard threshold-to-exact conversion. Hence computing \(|\mathcal{A}(Y)|\) is \#P-hard.
\end{proof}

\paragraph{Amount-linkage improvement.}
Under per-leaf conservation, an observer could reject candidate subsets whose individual deposit caps could not be routed to the published per-account totals. Aggregate conservation removes that pruning signal: the vector of authenticated inputs is visible only inside the local wrapper, and no output slot is publicly funded by a corresponding input. Candidate subsets are therefore filtered by total volume and slot count rather than a deposit-by-deposit transportation problem. This does not hide deposits or exits themselves, but it removes a deterministic amount-linkage channel between them.

\paragraph{Burn Cost and Bounded Rationality:}
A rational adversary may assume users are unwilling to leave significant value locked for privacy. Under bounded rationality, the effective anonymity set shrinks to plausible subsets whose total deposit volume is at most \((1 + \delta) \cdot S/(1-f)\) for some small \(\delta\) (e.g., 1\%)---that is, at most slightly above the minimum volume required by condition (iii).

\subsection{Privacy leakage}

Output amounts, exit addresses, nullifiers, and timing are public. The unlinkability bound in \cref{thm:unlinkability} assumes a uniform prior over the plausible deposit subsets \(\mathcal{A}(Y)\); behavioral amount patterns (e.g., round numbers), address reuse, deposit clustering, and timing correlation can shrink the effective set further. Aggregate enforcement hides the input-amount vector and its assignment to outputs, but the total-volume threshold remains inferable from \(S\) and \(f\). Nullifier indistinguishability (\cref{thm:nullifier-indist}) hides dummy padding but is orthogonal to deposit identification. First-layer padding and cross-leaf pooling enlarge plausible deniability, not full shielding.
...
We presented a hash-based authorization scheme where users commit funds to addresses derived from secret preimages and later claim them by proving knowledge of the preimage in zero knowledge. This approach sidesteps post-quantum signature verification entirely, requiring only hash evaluations and membership proofs inside the circuit. Recursive aggregation via Plonky2/FRI amortizes verification cost, while first-layer dummy padding and aggregate value enforcement reduce balance-linking information. We formalized the withdrawal and private-batch relations, proved six security properties via game-based definitions, showed that computing anonymity set size is \#P-hard, and demonstrated practical performance on commodity hardware.
