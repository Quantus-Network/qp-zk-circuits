...
\title[running={Post-Quantum Wormhole Proofs}]{Recursive Zero-Knowledge Proofs for Post-Quantum Private Settlement}
...
\keywords{zero-knowledge proofs, post-quantum cryptography, recursive aggregation, privacy, hash-based authorization}
...
We formalize this approach as a \emph{wormhole proof system}: funds are committed to addresses derived from secret preimages, then later claimed by proving knowledge of the preimage without revealing it. We construct a recursive proof architecture using Plonky2 with FRI commitments, enabling aggregation of many authorization proofs into a single succinct proof. To provide privacy against balance-linking adversaries, we introduce dummy padding with local shuffling at the first aggregation layer, enlarging the effective anonymity set.

We provide game-based security proofs for six properties: deposit binding, one-time withdrawal, spend-path exclusivity, value conservation, nullifier indistinguishability, and transaction unlinkability. Our analysis bounds adversarial advantage in terms of anonymity set size and shows that computing this size is \#P-hard. Benchmarks demonstrate 20\,ms proving time per authorization with 1.6\,ms verification, scaling to 256 authorizations via two-layer aggregation with constant-time (~3.7\,ms) final verification. The marginal on-chain cost is just 96 bytes per authorization---over 50$\times$ smaller than a single ML-DSA signature.
\end{abstract}
...
We formalize this approach as a wormhole proof system: funds are committed to addresses derived from secret preimages, then later claimed by proving knowledge of the preimage without revealing it. We construct a recursive proof architecture using Plonky2 with FRI commitments, enabling aggregation of many authorization proofs into a single succinct proof. To provide privacy against balance-linking adversaries, we introduce dummy padding with local shuffling at the first aggregation layer, enlarging the effective anonymity set.

We provide game-based security proofs for six properties: deposit binding, one-time withdrawal, spend-path exclusivity, value conservation, nullifier indistinguishability, and transaction unlinkability. Our analysis bounds adversarial advantage in terms of anonymity set size and shows that computing this size is #P-hard. Benchmarks demonstrate 20ms proving time per authorization with 1.6ms verification, scaling to 256 authorizations via two-layer aggregation with constant-time (3.7ms) final verification. The marginal on-chain cost is just 96 bytes per authorization—over 50x smaller than a single ML-DSA signature.
\end{textabstract}

\section{Introduction}

The post-quantum transition poses a dilemma for blockchains. Shor's algorithm breaks elliptic-curve signatures \cite{shor}, but post-quantum replacements like ML-DSA impose keys and signatures an order of magnitude larger \cite{mldsa}. A na\"ive swap increases bandwidth and state growth while doing nothing for privacy. Every transaction still reveals sender, recipient, and amount.
...
This paper takes a different approach: rather than proving signature validity, users prove knowledge of a secret tied to an unspent deposit. This idea appears in Ziggy \cite{ziggy}, a STARK-based signature scheme where signers prove knowledge of a hash preimage. We apply the same principle to blockchain settlement, building on EIP-7503's ``wormhole'' concept \cite{eip7503}: funds are sent to addresses derived from secrets and later withdrawn by proving knowledge of that secret. This sidesteps post-quantum signature verification entirely. The circuit only needs hash functions and storage-proof verification. We add recursive proof aggregation for scalable settlement.
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item Combinatorial analysis of anonymity set size under the first-layer grouping rules, with a proof that computing the anonymity set size is \#P-hard.
    \item Game-based security proofs for deposit binding, one-time withdrawal, spend-path exclusivity, value conservation, nullifier indistinguishability, and transaction unlinkability.
    \item Implementation and benchmarks demonstrating practical performance on commodity hardware.
\end{itemize}

\subsection*{Paper Organization}

\Cref{sec:background} reviews background on wormholes and transparent recursion. \Cref{sec:model} defines the system model and adversarial capabilities. \Cref{sec:overview} provides a protocol overview, followed by the formal specification in \Cref{sec:formal}. \Cref{sec:aggregation} describes the recursive aggregation architecture. \Cref{sec:security} presents security definitions and proofs. \Cref{sec:evaluation} reports benchmarks, and \Cref{sec:related} discusses related work.
...
The final aggregate proof is constant-size regardless of the number of authorizations. The marginal cost per authorization consists only of the public outputs that must be published: one exit address (32 bytes), one amount (32 bytes), and one nullifier (32 bytes), totaling \textbf{96 bytes per authorization}. For comparison, an ML-DSA-65 signature alone is 3,309 bytes, and a full post-quantum transaction with public key would exceed 5~KB. Our approach thus achieves a $>$50$\times$ reduction in per-authorization data while providing privacy guarantees that signatures cannot offer.
